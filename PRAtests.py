...
def comparison(rset,n,deltaset,N):
    ...
    for delta in deltaset:
        for r in rset:
            ...
            compsuccess = pd.concat([compsuccess,countseries],axis=1)            
            compCPU = pd.concat([compCPU,meanseries],axis=1)
            compCPU.to_csv('compCPU.csv')
            compsuccess.to_csv('compsuccess.csv')
            ...
            maxseries = normsdf.max(axis=1) 
            maxseries.name = '('+str(delta)+','+str(r)+','+str(n)+')'
            largestnorm = pd.concat([largestnorm,maxseries],axis=1)
            minseries = minevdf.min(axis=1) 
            minseries.name = '('+str(delta)+','+str(r)+','+str(n)+')'
            smallestminev = pd.concat([smallestminev,minseries],axis=1)

            compCPU.to_csv('compCPU.csv')
            compsuccess.to_csv('compsuccess.csv')
            compresults.to_csv('compresults.csv')

            largestnorm.to_csv('largestnorm.csv')
            smallestminev.to_csv('smallestminev.csv')
